-- Remove ALL existing policies for order_items to start fresh
...
DROP POLICY IF EXISTS "Users can update items for their orders" ON public.order_items;
DROP POLICY IF EXISTS "Users can delete items for their orders" ON public.order_items;

-- Create secure policy for viewing order items - only if user owns the order or is admin or anonymous order
CREATE POLICY "Secure order items view policy" 
...
CREATE POLICY "Secure order items insert policy" 
ON public.order_items 
FOR INSERT 
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Secure order items update policy" 
ON public.order_items 
FOR UPDATE 
USING (
  EXISTS (
...
CREATE POLICY "Secure order items delete policy" 
ON public.order_items 
FOR DELETE 
USING (
  EXISTS (
